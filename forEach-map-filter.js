// Reference from Springboard class 8 advanced array methods 8.1
...
function doubleValues(arr){
    let newDoubleValueArr = [];
    arr.forEach (function(val){newDoubleValueArr.push(val * 2)});
    return newDoubleValueArr;
}
/*
...
function onlyEvenValues(arr){
    let newEvenValuesArr = [];
    arr.forEach(function(val){
        if (val % 2 === 0){
            newEvenValuesArr.push(val);
        }
    });
    return newEvenValuesArr;
}
...
an array of objects, a key, 
and a value and returns the array passed to the 
function with the new key and value added for 
each object 

Examples:
    addKeyAndValue([{name: 'Elie'}, {name: 'Tim'}, 
    {name: 'Matt'}, {name: 'Colt'}], 'title', 
    'instructor') 
...
and returns an object with the keys 
as the vowel and the values as the number of times the vowel 
...
function vowelCount(str){
    ...
    splitArr.forEach(function(keyLetter){
        let lowerCaseL = keyLetter.toLowerCase();
        if(vowels.indexOf(lowerCaseL) !== -1){
            if(object[lowerCaseL]){
                object[lowerCaseL]++;
            }
            else {
                object [lowerCaseL] = 1;
...
/*
Write a function called extractFullName which accepts an 
array of objects and returns
 a new array with the value of the key with a name of "first" 
 and the value of a key 
 with the name of  "last" in each object, concatenated together 
 with a space. 
...
Examples:
    ...
    isCatOwner: true}], 'isCatOwner') // [{first: 'Tim', last:"Garcia", 
    isCatOwner: true}, 
...
a value and returns 
the first element in the array that has the same value 
as the second parameter or 
...
a key, 
and some value to search for and returns the first found value in 
the array.
...
array and returns a 
new array with all of the odd numbers doubled 
(HINT - you can use map and filter 
